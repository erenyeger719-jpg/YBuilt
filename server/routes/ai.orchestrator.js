...
function slugify(s) {
  return String(s || "").toLowerCase().replace(/[^a-z0-9 -]/g, "").trim().replace(/\s+/g, "-").slice(0,48);
}
function allowedFile(file) {
  return /^[a-zA-Z0-9._/-]+$/.test(file) && [".html",".htm",".css",".js"].some(ext => file.endsWith(ext));
}
function ensureIndexFallback(files, title="Page") {
  ...
  if (!names.has("styles.css")) files.push({ path:"styles.css", content:`:root{color-scheme:light dark}body{font-family:system-ui;margin:40px;line-height:1.6}`});
  if (!names.has("app.js"))     files.push({ path:"app.js", content:`console.log("Hello from app.js")`});
  return files;
}
...
router.post("/scaffold", express.json({limit:"4mb"}), async (req,res)=>{
  try{
    ...
    const system =
`You generate THREE web files from a plan/prompt. Output ONLY JSON:
...
- Respect tokens if provided: primary color, radius.`;
    ...
    files = files.filter(f => f?.path && f?.content).map(f => ({path:String(f.path), content:String(f.content)}));
    ...
    return res.json({ ok:true, path:`/previews/forks/${slug}/`, slug, model:{provider,model} });
  }catch(e){ res.status(500).json({error:e?.message||"scaffold failed"}); }
});
