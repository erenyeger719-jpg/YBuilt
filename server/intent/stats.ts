...
function scoreArm(sample: number, ema_ms?: number, ema_cents?: number, ema_tokens?: number) {
  ...
  const λ1 = 0.3, λ2 = 0.4, λ3 = 0.1; // tweak as you like
...
export function pickArm(): Arm {
  ...
  for (const arm of candidates) {
    const st = s[arm] || { a: 1, b: 1 };
    const draw = betaSample(Math.max(st.a, 0.5), Math.max(st.b, 0.5));
...
export function recordOutcome(arm: Arm, outcome: {
  success: boolean;
  ms?: number;         // wall time
  ...
  const s = load();
  const st = s[arm] || { arm, a: 1, b: 1, n: 0 };
...
export function underBudget({ cents = 0, maxCents = 0.5, tokens = 0, maxTokens = 4000 }) {
