...
// -------- default spec helper for /instant fallbacks --------
function buildDefaultInstantSpec(title?: string) {
  const safeTitle = (title || "Your new page").slice(0, 80);
    ...
    sections: [
      {
        type: "hero",
        headline: safeTitle,
        subheadline: "You just created this page with YBuilt Autopilot.",
        ...
        blocks: [
          {
            kind: "text",
            value:
              "Use the Magic Cursor to tweak this layout, add sections, and make it yours.",
          },
        ],
      },
      {
        type: "footer",
        text: "Built with YBuilt",
      },
    ],
  };
...
export function setupComposeRoutes(router: Router) {
    ...
    try {
      ...
      const out = await runWithBudget(sessionId, action, async (_tier) => {
        if (action.kind === "retrieve") {
          ...
          const specIntentAud = String(spec?.intent?.audience || "")
            .toLowerCase()
            .trim();
  ...
  router.post("/one", express.json(), async (req, res) => {
    try {
      ...
      // (Abbreviated: full /one compose logic lives in other modules)
  ...
  // ---------- /instant (zero-LLM compose, now with safe fallback) ----------
    ...
    try {
      ...
      if (sticky?.specId) {
        ...
        if (specCached) {
          ...
          const noJsSticky = decideNoJs(req, null);
      ...
      // If we reach here, build a safe default spec as a fallback.
      const fallbackSpec = buildDefaultInstantSpec(prompt);
...
<meta charset="utf-8">
<title>${escapeHtml(fallbackSpec.title || "Preview")}</title>
<body style="font-family:system-ui,-apple-system,Segoe UI,Roboto;max-width:800px;margin:40px auto;padding:24px;line-height:1.5">
<h1>${escapeHtml(
          (fallbackSpec.sections?.[0] as any)?.headline ||
            fallbackSpec.title ||
            "Preview",
        )}</h1>
<p>${escapeHtml(
          (fallbackSpec.sections?.[0] as any)?.subheadline ||
            "You just created this page with YBuilt Autopilot.",
        )}</p>
</body>`;
        ...
        // If preview write fails we still respond with a valid spec
      }
      ...
      return res.json({
        ...
        result: {
          pageId,
          path: `/previews/pages/${pageId}.html`,
        },
...
<p>${escapeHtml(
        ...
        const noJs = decideNoJs(req, null);
        const minimalSpec = buildDefaultInstantSpec(prompt);
